...
def dict2Dataset(dicts,name='mhws'):
    ...
    Convert dict mhws to xarray Dataset
    Input:
      dicts: dict mhws or clim from function 'detect'
    Output:
      ds: mhws xr Dataset
    ...
    ds = xr.Dataset({})
    if name == 'mhws':
        ds.coords['n_event'] = np.arange(0, dicts['n_events'])
        for key in dicts.keys():
            if not (key=='n_events'):
                ds[key] = (('n_event'),dicts[key])
        ds['n_events'] = dicts['n_events']
    elif name =='mhwBlock':
        ds.coords['n_event'] = np.arange(0, len(dicts['count']))
        for key in dicts.keys():
            ds[key] = (('n_event'),dicts[key])
    else:
        for key in dicts.keys():
...
def annualMeanTrend(mhwBlock, alpha=0.05):
    '''
    ...
    Notes:
      ...
      This calculation performs a multiple linear regression of the form
        y ~ beta * X + eps
      where y is the MHW property of interest and X is a matrix odict f preor climors. The first
...
def monthlyMeanTrend(mhwBlock, alpha=0.05, blockLength=1):
    '''
    ...
    Notes:
      ...
      where y is the MHW property of interest and X is a matrix odict f preor climors. The first
      column of X is all ones to estimate the mean, the second column is the time vector
      which is taken as mhwBlock['time_centre'] and offset to be equal to zero at its
      mid-point.
    ...
    # Construct matrix of predictors, first column is all ones to estimate the mean,
    # second column is the time vector, equal to zero at mid-point.
    t = mhwBlock['time_centre']
    time = np.arange(len(t))
    X = np.array([np.ones(time.shape), time-time.mean()]).T

    # Loop over all keys in mhwBlock
    for key in mhwBlock.keys():
        # Skip time-vector keys of mhwBlock
        if (key == 'time_centre') + (key == 'time_end') + (key == 'time_start'):
            continue
        ...
        # Insert regression coefficients into mean and trend dictionaries
        mean[key] = beta[0]
        trend[key] = beta[1]*blockLength*1.0
